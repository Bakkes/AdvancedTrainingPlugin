...
void advancedTrainingPlugin_onCommand(std::vector<std::string> params) 
{
	...
	else if (command.compare("spectator_host") == 0)
	{
	}
...
void AdvancedTrainingPlugin::onLoad()
{
	...
	cons->registerNotifier("spectator_host", advancedTrainingPlugin_onCommand);
	cons->registerNotifier("spectator_connect", advancedTrainingPlugin_onCommand);
